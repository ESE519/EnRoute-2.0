...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_ON_OFF_LAZY_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_ON_OFF_LAZY_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_ON_OFF_LAZY_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "ON_OFF_LAZY") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_ON_OFF_LAZY_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_ON_OFF_LAZY_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "ON_OFF_LAZY") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_ON_OFF_LAZY_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_ON_OFF_LAZY_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"ON_OFF_LAZY")) {
    return(sf_ON_OFF_LAZY_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void ON_OFF_LAZY_register_exported_symbols(SimStruct* S);
extern void ON_OFF_LAZY_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  ON_OFF_LAZY_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      ON_OFF_LAZY_debug_initialize();
    }

    ON_OFF_LAZY_initializer();
  }
